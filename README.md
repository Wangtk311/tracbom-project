# ★Tracbom 音·迹  4键下落式音乐节奏游戏

<img width="130" alt="game - Copy" src="https://github.com/Wangtk311/tracbom-project/assets/115420227/650a7137-f708-4483-b82b-d90de6b9e824">
...
**如果您认为该项目侵犯了您的版权，请联系我们，确认属实后我们将立即删除侵权内容**

2023.12.29提交 开发时长约2周 采用`MIT`许可 项目截图见最下方

开发人员共2人：

**`吴忱`** 项目组长，设计了游戏的核心游玩功能(贡献50-55%)

**`王天阔`** 设计了游戏的美工、场景切换、参数调整和选歌等外围功能(贡献45-50%)

要联系我们，请使用此E-mail： `2250736@tongji.edu.cn`

-------

+ **更新暂停**

`2024.1.12` 此项目暂时停止维护。但是其原有的支持将不会停止。如果继续更新，我们将在这里进行通知。
...
`2024.1.12` 添加曲目：Äventyr、MEGAMIX、Timeline、R、ニヒルと水没都市 五首。

历史更新：

`2024.1.12` 修复了全部完美击键时分数不是1000000的Bug。

`2024.1.6` 添加曲目：Another Me、c.s.q.n. 两首。

`2024.1.5` 修复了最大连击数计算和显示错误的Bug。
...
**无论如何，请您确保文件保存在一个不含有`中文字符`的路径下，最好是`纯英文字符`和`数字`的组合。**

方法一：
如果您只是想游玩，可以直接克隆`package`分支(可用，但不建议)，或者在发布版本中选择下载的版本。在`Releases`中(或`Tags`中)保存有游戏包版本，版本号的`release`代表正式版，`beta`代表测试版，这些是直接可以游玩的版本，下载解压后双击`tracbom_release`文件夹下的`tracbom.exe`即可游玩。如果遇到无声、画面异常等问题，请使用方法二获取完整项目并编译运行，如果问题仍然存在，请联系我们。
...
**需要注意，我们对游戏音效引擎SimpleAudioEngine进行过修改**(范围为MciPlayer.cpp，MciPlayer.h，SimpleAudioEngine.cpp)**，请勿仅复制Resources和Classes以创建项目，以免引起音效引擎异常！**
...
建议您每次游玩时，都根据设备情况，将延迟调整为合适的水平，以优化您的游玩体验。
...
项目的核心是`竞技性`、`灵活性`、`扩展性`和`现代性`。我们希望制作一款游戏，它的玩法非常简单，不需要经过繁难的学习，但`想要玩好却非常不容易`，这是一种考验`技术力`的游戏。另外，我们希望这款游戏可以适应每一个人，每人可以`根据喜好调整键位和反馈等设置`，以达到最佳游戏体验。之后，我们希望它是可扩展的，用户可以`根据喜好添加或删除歌曲的谱面，同时又可以创作属于自己的谱面`，打造一款`个性化`的游戏。最后，我们希望它的界面和设计等是现代化的，符合`新时代游戏设计审美`。

此外，在transtools目录下，我们提供了`将已有的malody谱面和osu谱面转换为tracbom谱面的转换器`(两种转换器均只能转换4键下落玩法版本的谱面，请确认转换谱面的格式正确)，以便于添加谱面。具体的使用方法，您可以联系我们了解。

------

+ **项目功能介绍**

- [x] 选项设置：背景音量、音效音量、按键延迟、键位调整。

- [x] 游玩功能：短击键、长按键、偏移判定、暂停和重玩、轨道的消失、出现、移动、旋转、缩放等。

- [x] 评级功能：连击计数、Perfect、Good、Bad、Miss击键判定、等级评定。

- [x] 记录存储：存储游戏属性、高分记录等。

- [x] 谱面管理：添加谱面、malody和osu的已有谱面转换。
...
如果您没有现成的谱面，那么自创谱面将会比较困难，虽然这是可行的，但我们并不推荐您这样做。项目根目录下描述了添加音符和轨道的方法供您查阅。
