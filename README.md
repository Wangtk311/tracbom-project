...
方法一：
如果您只是想游玩，可以直接拉取`package`分支，或者在发布版本中选择`beta-v0.0.1`，这是直接可以游玩的版本，双击文件夹下的`tracbom.exe`即可游玩。如果遇到无声、画面异常等问题，请`使用方法二获取完整项目并编译运行`。

方法二：
如果您想要完整的项目：
该GitHub存储库所保存的是`Visual Studio 2022`项目。您可以使用VS克隆此存储库，或者直接下载库对应的zip后缀压缩包。如果您选择`直接使用VS2022打开`，请在启动项目中选择`tracbom`项目，然后开始编译和执行。如果有提示工具集问题，请使用Visual Studio推荐的设置。如果您下载的是`zip压缩包`，请解压该压缩包，进入`tracbom\proj.win32`路径下，用VS装载`tracbom.sln`解决方案即可，之后的步骤与前一种方案相同。
