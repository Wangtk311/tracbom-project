...
方法一：
如果您只是想游玩，可以直接克隆`package`分支(可用，但不建议)，或者在发布版本中选择下载的版本。在`Tags`中(或`Releases`中)保存有游戏包版本，版本号的`release`代表正式版，`beta`代表测试版，这些是直接可以游玩的版本，下载解压后双击`tracbom_release`文件夹下的`tracbom.exe`即可游玩。如果遇到无声、画面异常等问题，请`使用方法二获取完整项目并编译运行`，如果问题仍然存在，请联系我们。
