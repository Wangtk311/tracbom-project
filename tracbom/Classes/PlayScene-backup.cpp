...
bool Play::init()
{
	...
	std::string effectFilename = "sounds/tapEffect.wav";
	...
	this->addChild(scoreLab,10);
	scoreLab->setPosition(Vec2(visibleSize.width + origin.x-200, visibleSize.height+origin.y -70));

	auto comboLab = Label::createWithTTF("", "fonts/combo.otf", 70);
	this->addChild(comboLab,10);
	...
	listener->onKeyReleased =[pauseLayer, pauseMenu](EventKeyboard::KeyCode keyCode, Event* event)->void {
		if (keyCode == EventKeyboard::KeyCode::KEY_ESCAPE/* && !Play_End && !Play_TimeStop*/ ){
			...
			//Director::getInstance()->getScheduler()->pauseTarget();// 暂停目标对象的所有调度器
			//GamePause(this);
		}
		...
		if (keyCode == EventKeyboard::KeyCode::KEY_ENTER /* && Play_End*/){
			...
			pauseTostarts.pushBack(SpriteFrame::create("1.png", Rect(0, 0, 40, 40)));
			pauseTostarts.pushBack(SpriteFrame::create("2.png", Rect(0, 0, 40, 40)));
			pauseTostarts.pushBack(SpriteFrame::create("3.png", Rect(0, 0, 40, 40)));
			...
			//Director::getInstance()->getScheduler()->resumeTarget(this); // 恢复目标对象的所有调度器
			//backmeun(this);
		}
		//恢复按键锁定
...
void Play::reTry() {
	...
	//Director::getInstance()->popScene();
	Director::getInstance()->pushScene(playSce);
	//log("Time: %f" , Time); 
}
